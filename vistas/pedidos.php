...
<script type="text/javascript">
  ...
  $.ajax({
    ...
    success: function(data) {
      if (Array.isArray(data) && data.length > 0) { // Verifica si la API devolvió datos válidos
        console.log('Datos de todos los clientes:', data);
        ...
        if (cliente) {
          ...
          // Obtén los datos necesarios del cliente
          var fecha = obtenerFechaActual(); // Puedes ajustar según tus necesidades
          var nombre = cliente.nombre;
          var apellido = cliente.apellido;

          // Lógica para registrar el pedido en tu base de datos
          $.ajax({
            url: '../procesos/pedidos/agregaPedido.php',
            type: 'POST',
            data: { fecha: fecha, nombre: nombre, apellido: apellido, dni: dniCliente },
            success: function(response) {
              console.log('Respuesta del servidor:', response);

              // Después de registrar el pedido, muestra un mensaje de éxito
              alertify.success('Pedido agregado con éxito');
            ...
            error: function(error) {
              ...
              // Muestra un mensaje de fallo al agregar pedido
              alertify.error('Fallo al agregar pedido. Verifica la consola para más detalles.');
            }
          });
        } else {
          console.error('Cliente no encontrado con DNI:', dniCliente);

          // Muestra un mensaje de fallo al agregar pedido debido a DNI no encontrado
          alertify.error('Fallo al agregar pedido. DNI no encontrado.');
        }
      } else {
        console.error('Datos inválidos de la API:', data);

        // Muestra un mensaje de fallo al obtener datos de la API
    ...
    error: function(xhr, status, error) {
      ...
      // Muestra un mensaje de fallo al obtener datos de todos los clientes
